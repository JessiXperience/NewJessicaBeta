...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void updateRenderer()
    {
        ...
        this.thirdPersonDistancePrev = ModuleManager.getModule("CameraClip").isToggled() ? (float)CameraClip.fov.getDoubleValue() / 8.0F : 4.0F;
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null)
        {
            ...
            RayTraceResult rayTraceResult = entity.rayTrace(d0, partialTicks);
            boolean canInteract = true;
            if (ModuleManager.getModule("NoInteract").isToggled() && rayTraceResult != null && rayTraceResult.typeOfHit == RayTraceResult.Type.BLOCK && rayTraceResult.getBlockPos() != null) {
                for (Block block : NoInteract.blocks) {
                    if (Wrapper.world().getBlockState(rayTraceResult.getBlockPos()).getBlock() == block) {
                    	canInteract = false;
                    }
                }
            }
            if (canInteract) {
                this.mc.objectMouseOver = rayTraceResult;
            }
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting)
    {
        ...
        else
        {
            ...
            if (flag && !ModuleManager.getModule("Zoom").isToggled())
            {
                if (!Config.zoomMode)
                {
            ...
            if(ModuleManager.getModule("Zoom").isToggled()) {
            	f /= Zoom.scale;
            }
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
        	...
        	double d3 = this.thirdPersonDistancePrev;
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (raytraceresult != null && !ModuleManager.getModule("CameraClip").isToggled())
                    {
                ...
                float clipX = ModuleManager.getModule("CameraClip").isToggled() ? (float)CameraClip.x.getDoubleValue() : 0.0F;
                float clipY = ModuleManager.getModule("CameraClip").isToggled() ? (float)CameraClip.y.getDoubleValue() : 0.0F;
                GlStateManager.rotate(entity.rotationPitch - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(entity.rotationYaw - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(clipX, clipY, (float)(-d3));
    ...
    public void setupCameraTransform(float partialTicks, int pass)
    {
        ...
        if (f1 > 0.0F && !ModuleManager.getModule("AntiBadEffects").isToggled())
        {
            int i = 20;
    ...
    {
    	if (ModuleManager.getModule("Tracers").isToggled() && Tracers.mode.getValue().equalsIgnoreCase("Tracer")) {
            for (Entity entity : Wrapper.world().getLoadedEntityList()) {
                if (!(entity instanceof EntityLivingBase)) continue;
                Profiler.renderProfiler(entity, p_renderHand_1_, false, false, false, false, false, false, true, false, false);
            }
        }
        if (!this.debugView)
        {
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    if (this.mc.player.isPotionActive(MobEffects.NIGHT_VISION) || ModuleManager.getModule("NightVision").isToggled())
                    {
                    ...
                    this.lightmapColors[i] = ModuleManager.getModule("Ambience").isToggled() ? Ambience.color.getValue() : -16777216 | k << 16 | l << 8 | i1;
                }
    ...
    {
    	if(ModuleManager.getModule("NightVision").isToggled()) {
    		return 1.0f;
    	}
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            else
            {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null)
                {
                    ...
                    ModuleManager.onRenderGameOverlay(partialTicks);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        RenderWorldLastEvent event = new RenderWorldLastEvent(this.mc.renderGlobal, partialTicks);
        ModuleManager.onRenderWorldLastEvent(event);
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        else if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS) && !ModuleManager.getModule("AntiBadEffects").isToggled())
        {
            float f2 = 5.0F;
            int i = ((EntityLivingBase)entity).getActivePotionEffect(MobEffects.BLINDNESS).getDuration();
