...
public class GuiIngame extends Gui
{
    ...
    {
    /*	int lasttime = this.tick;
        if (this.counter.get() > 1000L) {
            ++this.tick;
            this.counter.reset();
            Wrapper.getFiles().saveValues();
        }
        if (HUD.totaltime.getValue()) {
            if (lasttime != this.tick) {
                final DecimalFormat decimalFormatHour = new DecimalFormat("0");
                final DecimalFormat decimalFormat = new DecimalFormat("00");
                Display.setTitle(String.valueOf(Wrapper.getClientName()) + " " + Wrapper.getVesrion() + " Time: " + String.valueOf(String.valueOf(decimalFormatHour.format(this.tick / 3600)) + ":" + decimalFormat.format(this.tick % 3600 / 60) + ":" + decimalFormat.format(this.tick % 60)));
            }
        }
        else {
            Display.setTitle(String.valueOf(String.valueOf(Wrapper.getClientName())) + " " + Wrapper.getVesrion());
        }
    */
        ...
        if (this.titlesTimer > 0)
        {
            ...
            if (i2 > 8)
            {
                ...
                GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                GlStateManager.pushMatrix();
                if (!this.displayedTitle.contains("Функция ")) {
                    GlStateManager.scale(4.0f, 4.0f, 4.0f);
                }
                int j2 = i2 << 24 & 0xFF000000;
                if (this.displayedTitle.contains("Функция ")) {
                    this.getFontRenderer().drawString(this.displayedTitle.replace("Функция ", ""), -this.getFontRenderer().getStringWidth(this.displayedTitle.replace("Функция ", "")) / 2, 50, this.displayedTitle.contains("enabled") ? Color.GREEN.getRGB() : Color.RED.getRGB());
                }
                else {
