...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void sendChatMessage(final String message) {
        if (message.startsWith(".say ")) {
            ChatCommands.commands(message);
            return;
        }
        if (!message.startsWith(".bind ") && message.contains("{near}")) {
            try {
                this.sendChatMessage(message.replace("{near}", EntityUtils.getClosestPlayer().getName()));
            }
            catch (Exception e) {
        ...
        if (!message.startsWith(".bind ") && message.contains("{from_list}")) {
            new Thread() {
                @Override
                public void run() {
                    try {
                        Reader br2 = null;
                        File fr = null;
                        fr = new File(ChatCommands.fileList);
                        br2 = new BufferedReader(new InputStreamReader(new FileInputStream(fr), "UTF-8"));
                        BufferedReader br3 = new BufferedReader(br2);
                        String sCurrentLine2;
                        while ((sCurrentLine2 = br3.readLine()) != null) {
                            EntityPlayerSP.this.sendChatMessage(message.replace("{from_list}", sCurrentLine2));
                            Thread.sleep(ChatCommands.delayList);
                        }
                    }
                    catch (Exception e) {
                        Wrapper.msg("&cError!", true);
        ...
        if (!message.startsWith(".bind ") && message.contains("{players}")) {
            new Thread() {
                @Override
                public void run() {
                    try {
                        ...
                        for (final NetworkPlayerInfo n : players) {
                            if (Wrapper.player().getGameProfile().getName().equals(n.getGameProfile().getName())) {
                                continue;
                            }
                            EntityPlayerSP.this.sendChatMessage(message.replace("{players}", n.getGameProfile().getName()));
                            Thread.sleep(ChatCommands.delayList);
                        }
                    }
                    catch (Exception e) {
    ...
    public void onLivingUpdate()
    {
        ...
        InputUpdateEvent event = new InputUpdateEvent(this, movementInput);
        ModuleManager.onInputUpdate(event);
